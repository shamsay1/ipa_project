 ...
 <div id="content">
    ...
    <div id="solutionsList">
        ...
        <div class="table-container p-3">
            ...
            <form id="editForm" action="{{ route('timetable.update', $timetable->id)}}" method="post">
    @csrf
    @method('PUT')
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <select name="timeslot_id" id="timeslot_id" class="form-select" oninput="checkConflicts()">
                @foreach ($time as $ti)
                <option value="{{ $ti->id}}" {{ $timetable->timeslot_id == $ti->id ? "selected" : ""}}>
...
function loadSolutions() {
    ...
    fetch('{{ route("timetable.checkSolutions") }}', {
        method: 'POST',
        headers: {
...
function checkConflictsForButton() {
    ...
    fetch('{{ route("timetable.checkConflicts") }}', {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "X-CSRF-TOKEN": "{{ csrf_token() }}"
