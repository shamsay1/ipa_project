...
<div id="content">
    ...
    <div class="col-md-7 d-flex flex-wrap align-items-center mb-3 mb-md-0 gap-2">
        <form action="{{ route('subject.import') }}" method="POST" enctype="multipart/form-data" id="importForm" style="display:none;">
            @csrf
        ...
        <a href="{{ route('subject.template')}}" class="btn btn-info text-white">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select class="form-select" name="semester" required>
                                        ...
                                        <option value="{{ $sem->id }}">{{ $sem->semName }} {{ $sem->academic_year }}</option>
                                            
                                        @endforeach
                                        {{-- @if($semester)
                                        <option value="{{ $semester->id }}" selected>{{ $semester->semName }}</option>
                                        @else
...
data-bs-target="#course-deg-{{ $course->id }}">
...
@foreach($subjectsByNta as $nta => $subjects)
...
<button class="accordion-button collapsed"
data-bs-toggle="collapse"
data-bs-target="#deg-nta-{{ $course->id }}-{{ $nta }}">
...
@foreach($subjectsBySemester as $semesterName => $semesterSubjects)

<h6 class="text-success mt-3">{{ $semesterName }}</h6>
...
<button class="accordion-button collapsed"
data-bs-toggle="collapse"
data-bs-target="#course-dip-{{ $course->id }}">
...
<button class="accordion-button collapsed"
data-bs-toggle="collapse"
data-bs-target="#dip-nta-{{ $course->id }}-{{ $nta }}">
